        ...
        for C in [0.1, 1, 10, 100, 1000]:
            for gamma in ["scale", "auto", 10, 1, 0.1, 0.01, 0.001]:
                ...
                for kernel in ["rbf", "linear"]:
                    ...
                    logger.info(
                        f'Accuracy: {this_results["acc"]}, Macro-F1: {this_results["macro-F1"]}'
                    )
    else:
        if classifier == "SVM":
            clf = SVC(gamma=gamma)
...
def crosslingual_classification(
    ...
    intra_en = evaluate_classification(
        train_theta_en,
        test_theta_en,
        train_labels_en,
        test_labels_en,
    ...
    intra_cn = evaluate_classification(
        train_theta_cn,
        test_theta_cn,
        train_labels_cn,
        test_labels_cn,
    ...
    cross_en = evaluate_classification(
        train_theta_cn,
        test_theta_en,
        train_labels_cn,
        test_labels_en,
    ...
    cross_cn = evaluate_classification(
        train_theta_en,
        test_theta_cn,
        train_labels_en,
        test_labels_cn,
...
def hierarchical_classification(
    train_theta, test_theta, train_labels, test_labels, classifier="SVM", gamma="scale"
):
    num_layer = len(train_theta)
    results = defaultdict(list)

    for layer in range(num_layer):
        layer_results = evaluate_classification(
            train_theta[layer],
            test_theta[layer],
            train_labels,
            test_labels,
