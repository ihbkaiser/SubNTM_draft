...
def evaluate_irbo(top_words: list[list[str]], topk: int = 10, weight: float = 0.9) -> float:
    """
    Inverted Rank-Biased Overlap (IRBO) of top words
    """
    if not top_words:
    ...
    if topk > min_len:
        raise ValueError(f"topk={topk} > ({min_len}).")
